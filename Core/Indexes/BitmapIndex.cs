...
namespace RaptorDB
{
    internal class BitmapIndex : IDisposable
    {
            ...
            lock (_oplock)
                lock (_readlock)
                    lock (_writelock)
                    {
                        ...
                        var _newrec = new FileStream(_Path + _FileName + "$" + _recExt, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite);
                        var _newbmp = new FileStream(_Path + _FileName + "$" + _bmpExt, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite);
        ...
        private void InternalShutdown()
        {
            ...
            {
                Flush();
                if (_recordFileWrite.Length == 0) {
                    d1 = true;
                }


                if (_bitmapFileWrite.Length == 0) {
                    d2 = true;
                }
